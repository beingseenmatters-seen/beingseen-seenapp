...
// Only the four visible fields participate in dirty-checking; hidden legacy
// fields (location, currentState, interests) are preserved untouched on save.
...
export default function BasicProfile() {
  ...
  const isDirty = useMemo(() => {
    const current = normalizeProfileForCompare({ nickname, age, gender, zodiac });
    return JSON.stringify(current) !== savedSnapshot;
  }, [nickname, age, gender, zodiac, savedSnapshot]);
  ...
  const handleSave = async () => {
    ...
    // Spread the existing stored basic profile first so hidden legacy fields
    // (location, currentState, interests, …) are never deleted or overwritten.
    const nextBasic = {
      ...(seenUser?.basic || {}),
